...
<div class="px-3 py-4">
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-6 nilai-header gap-4">
        <div>
            ...
            <p class="text-sm sm:text-base text-gray-600">{{ $kelas->nama_kelas ?? 'Kelas Tidak Diketahui' }} - {{ $mapel->nama ?? 'Mata Pelajaran Tidak Diketahui' }}</p>
        ...
        <a href="{{ route('guru.nilai.show', $kelas->id) }}" class="bg-gray-100 hover:bg-gray-200 text-gray-700 py-2 px-4 rounded-lg transition-colors text-center sm:text-left">
            ...
            <form action="{{ route('guru.nilai.store') }}" method="POST">
                ...
                <div class="mb-4 form-row">
                    <div class="w-full px-3 py-2 border border-gray-300 rounded-lg bg-gray-50 text-gray-700">
                        <strong>Kelas:</strong> {{ $kelas->nama_kelas ?? 'Kelas Tidak Diketahui' }} | 
                        <strong>Mata Pelajaran:</strong> {{ $mapel->nama ?? 'Mata Pelajaran Tidak Diketahui' }}
                    </div>
                </div>
