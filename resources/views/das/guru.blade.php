        ...
        <div class="relative p-4 sm:p-6 lg:p-8">
            ...
            <div class="relative z-10">
                <h1 class="text-xl sm:text-2xl lg:text-3xl font-bold text-white mb-2">
                    Selamat {{ now()->format('H') < 12 ? 'Pagi' : (now()->format('H') < 15 ? 'Siang' : (now()->format('H') < 18 ? 'Sore' : 'Malam')) }}, {{ auth()->guard('guru')->user()->nama_lengkap }}!
                </h1>
                ...
                <div class="p-3 sm:p-5">
                    ...
                    @if($groupedJadwal->count() > 0)
                        <div class="grid grid-cols-2 lg:grid-cols-1 gap-3 sm:gap-4">
                            ...
                            <div class="bg-gray-50 rounded-lg p-3 hover:bg-blue-50 transition-all">
                                <div class="flex items-center justify-between mb-2">
                                    <div class="h-8 w-8 sm:h-10 sm:w-10 bg-blue-100 rounded-lg flex items-center justify-center text-blue-500 flex-shrink-0">
                                        <span class="font-medium text-xs">{{ substr($group['jam_mulai'], 0, 5) }}</span>
                                    </div>
                                    <div class="text-xs text-gray-600 text-right">
                                        {{ substr($group['jam_mulai'], 0, 5) }} - {{ substr($group['jam_selesai'], 0, 5) }}
                                        @if(count($group['items']) > 1)
                                            <div class="text-xs text-blue-600 font-medium">{{ count($group['items']) }} JP</div>
                                        @endif
                                    </div>
                                </div>
                                <div class="mb-3">
                                    <h4 class="font-medium text-gray-900 text-xs sm:text-sm mb-1 line-clamp-2">{{ $group['mapel'] ? $group['mapel']->nama : 'Mata Pelajaran tidak ditemukan' }}</h4>
                                    <div class="flex items-center text-xs text-gray-600">
                                        <i class="fas fa-users mr-1"></i>
                                        <span class="truncate">{{ $group['kelas'] ? $group['kelas']->nama_kelas : 'Kelas tidak ditemukan' }}</span>
                                    </div>
                                </div>
                                <div class="flex gap-1">
                                    <a href="{{ route('guru.absensi.create', ['jadwal_id' => $group['items'][0]->id]) }}" 
                                       class="flex-1 text-center px-2 py-1.5 bg-blue-600 hover:bg-blue-700 text-white text-xs rounded-lg transition-all">
