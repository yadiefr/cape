...
<?php $__env->startSection('title', 'Jadwal Pelajaran - SMK PGRI CIKAMPEK'); ?>
        ...
        <div class="bg-yellow-50 border-l-4 border-yellow-400 p-4 mb-6 rounded-r-lg">
            <div class="flex">
                ...
                <div class="ml-3">
                    ...
                    <p class="text-sm text-yellow-700 mt-1"><?php echo e($error); ?></p>
                </div>
            </div>
        </div>
    <?php else: ?>
            ...
            <div class="bg-white rounded-lg shadow-sm overflow-hidden">
                <div class="px-6 py-4 bg-gradient-to-r from-indigo-600 to-purple-700 text-white">
                    <div class="flex justify-between items-center">
                        <div>
                            ...
                            <?php endif; ?>
                            <?php if($settingsJadwal->isNotEmpty()): ?>
                            ...
                            <div class="text-indigo-100 text-xs mt-2 flex flex-wrap gap-2">
                                ...
                                ?>
                                <?php if($daySettings->jam_istirahat_mulai && $daySettings->jam_istirahat_selesai): ?>
                                    <span class="px-2 py-1 bg-white bg-opacity-20 rounded">
                                        <i class="fas fa-coffee mr-1"></i> Istirahat 1: <?php echo e(\Carbon\Carbon::parse($daySettings->jam_istirahat_mulai)->format('H:i')); ?> - <?php echo e(\Carbon\Carbon::parse($daySettings->jam_istirahat_selesai)->format('H:i')); ?>

                                    </span>
                                <?php endif; ?>
                                <?php if($daySettings->jam_istirahat2_mulai && $daySettings->jam_istirahat2_selesai): ?>
                                    <span class="px-2 py-1 bg-white bg-opacity-20 rounded">
                                        <i class="fas fa-utensils mr-1"></i> Istirahat 2: <?php echo e(\Carbon\Carbon::parse($daySettings->jam_istirahat2_mulai)->format('H:i')); ?> - <?php echo e(\Carbon\Carbon::parse($daySettings->jam_istirahat2_selesai)->format('H:i')); ?>
                ...
                <div class="overflow-x-auto">
                    <table class="w-full schedule-table text-sm">
                        <thead class="bg-gray-50">
                            <tr>
                                ...
                                <?php $__currentLoopData = ['Senin', 'Selasa', 'Rabu', 'Kamis', 'Jumat']; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $day): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                    ...
                                    ?>
                                    <th class="px-2 py-2 text-center text-xs font-medium text-gray-500 uppercase tracking-wider border-r border-gray-200 <?php echo e($isToday ? 'today-header' : ''); ?>">
                                        <?php if($isToday): ?><div class="today-indicator"></div><?php endif; ?>
                                        <div class="flex items-center justify-center">
                                            <div class="day-indicator <?php echo e(strtolower($day)); ?>-color"></div>
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            ...
                            <?php if(count($jam_slots) > 0): ?>
                                <?php $__currentLoopData = $jam_slots; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $jam_key => $hari_jadwal): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                <?php
                                ...
                                <tr class="hover:bg-gray-50 <?php echo e($isBreakTime ? 'bg-amber-50' : ''); ?>">
                                    <td class="px-2 py-2 text-xs font-bold text-gray-900 border-r border-gray-200 bg-gray-50 sticky left-0 <?php echo e($isBreakTime ? 'bg-amber-100' : ''); ?>">
                                        <div class="flex flex-col">
                                            <span class="text-xs font-semibold"><?php echo e($jam_key); ?></span>
                                            <span class="text-xs text-gray-500">
                                                <?php if($isBreakTime): ?>
                                                    <span class="text-amber-600 font-medium text-xs"><i class="fas fa-coffee mr-1"></i><?php echo e($breakLabel); ?></span>
                                                <?php else: ?>
                                                    <?php
                                    ...
                                    ?>
                                    <td class="px-1 py-1 text-xs border-r border-gray-200 text-center min-w-[100px] md:min-w-[140px] align-top <?php echo e($isToday ? 'today-column' : ''); ?>" style="height: auto !important;">
                                        <?php if(isset($all_time_slots[$jam_key]['is_break']) && $all_time_slots[$jam_key]['is_break']): ?>
                                            <div class="break-time-cell">
                                                <i class="fas <?php echo e($all_time_slots[$jam_key]['break_label'] == 'Istirahat 1' ? 'fa-coffee' : 'fa-utensils'); ?> text-xs"></i>
                                                <span class="break-time-label text-xs"><?php echo e($all_time_slots[$jam_key]['break_label']); ?></span>
                                            </div>
                                        <?php elseif(isset($hari_jadwal[$hari]) && count($hari_jadwal[$hari]) > 0): ?>
                                            <div class="space-y-1 max-h-48 md:max-h-48 overflow-y-auto" style="min-height: auto !important;">
                                                <?php $__currentLoopData = $hari_jadwal[$hari]; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $item): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                                    <?php
                                                    ...
                                                    ?>
                                                    <div class="schedule-cell <?php echo e($color_class); ?> border rounded-lg p-1 mb-1 hover:shadow-md transition-all cursor-pointer" title="Mata Pelajaran: <?php echo e($mapelNama); ?>">
                                                        <?php if(isset($item->is_break) && $item->is_break): ?>
                                                        ...
                                                        <?php elseif(isset($item->mapel)): ?>

                                                            <div class="text-xs font-medium mb-1 truncate mapel-name" title="<?php echo e($item->mapel->nama); ?>">
                                                                <i class="fas fa-book text-xs mr-1 opacity-75"></i><?php echo e($item->mapel->nama); ?>

                                                            </div>
                                                            <?php if(isset($item->guru)): ?>
                                                                <div class="text-xs text-gray-600 truncate guru-name" title="<?php echo e($item->guru->nama); ?>">
                                                                    <i class="fas fa-user text-xs mr-1 opacity-75"></i><?php echo e($item->guru->nama); ?>

                                                                </div>
                                                            <?php endif; ?>
                                                            <?php if($item->keterangan): ?>
                                                                <div class="mt-1 text-xs text-gray-500 truncate hidden lg:block" title="<?php echo e($item->keterangan); ?>">
                                                                    <i class="fas fa-info-circle mr-1"></i><?php echo e($item->keterangan); ?>

                                                                </div>
                                                            <?php endif; ?>
                                                            <?php if(isset($item->ruangan)): ?>
                                                                <div class="mt-1 text-xs bg-white bg-opacity-50 text-gray-600 inline-block px-1 py-0.5 rounded hidden sm:block">
                                                                    <i class="fas fa-door-open mr-1"></i><?php echo e($item->ruangan); ?>
                ...
                <div class="mt-6 bg-gray-50 rounded-lg p-4">
                    ...
                    <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-2 text-xs">
                        <?php $__currentLoopData = $classMapel; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $mapelNama): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                            <?php if(isset($mapel_colors[$mapelNama])): ?>
                                <div class="flex items-center space-x-2">
                                    <div class="w-4 h-4 rounded border <?php echo e($mapel_colors[$mapelNama]); ?>"></div>
                                    <span class="text-gray-700 truncate" title="<?php echo e($mapelNama); ?>"><?php echo e($mapelNama); ?></span>
                                </div>
                            <?php endif; ?>
                        <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                    </div>
...
<?php echo $__env->make('layouts.siswa', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\wamp64\www\website-smk3\resources\views/siswa/jadwal/index.blade.php ENDPATH**/ ?>