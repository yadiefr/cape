...
<?php $__env->startSection('title', 'Manajemen Siswa - SMK PGRI CIKAMPEK'); ?>
...
<div class="bg-white rounded-xl shadow-md p-6 mb-6">
    <div class="flex flex-col space-y-6">
            ...
            <div class="relative z-10">
                <div class="flex flex-col md:flex-row justify-between items-start md:items-center">
                    ...
                    <div class="mt-4 md:mt-0">
                        <a href="<?php echo e(route('admin.siswa.create')); ?>" class="btn btn-success btn-sm">
                    ...
                    <div class="bg-gradient-to-br from-green-50 to-green-100 p-5 rounded-xl shadow-sm hover:shadow-md transition-all border border-green-200 relative overflow-hidden group">
                        ...
                        <div class="relative z-10">
                            ...
                            <div class="flex flex-col sm:flex-row gap-2 mb-3">
                                ...
                                <a href="<?php echo e(route('admin.siswa.template')); ?>" 
        ...
        <div id="filterPanel" class="bg-gray-50 rounded-xl border border-gray-200 p-5 hidden">
            <form action="<?php echo e(route('admin.siswa.index')); ?>" method="GET" class="grid grid-cols-1 md:grid-cols-4 gap-4">
                <!-- Hidden fields to preserve sorting -->
                <input type="hidden" name="sort_by" value="<?php echo e(request('sort_by', 'siswa.nama_lengkap')); ?>">
                <input type="hidden" name="sort_order" value="<?php echo e(request('sort_order', 'asc')); ?>">
                ...
                <div class="md:col-span-4 flex justify-end space-x-2">
                    ...
                    <a href="<?php echo e(route('admin.siswa.index')); ?>" class="bg-gray-200 text-gray-700 px-4 py-2 rounded-lg hover:bg-gray-300 transition-all flex items-center">
        ...
        <!-- Students Table -->
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">            <div class="overflow-x-auto">                <form id="bulkDeleteForm" action="<?php echo e(route('admin.siswa.bulkDelete')); ?>" method="POST">
                    <?php echo csrf_field(); ?>
                    ...
                    <table class="min-w-full divide-y divide-gray-200">                        <thead class="bg-gray-50">
                            <tr>
                                ...
                                <th scope="col" class="px-6 py-3 text-center text-xs font-medium text-gray-500 uppercase tracking-wider">
                                    ...
                                    <div class="inline-flex ml-2">
                                        <a href="<?php echo e(request()->fullUrlWithQuery(['sort_by' => 'siswa.nama_lengkap', 'sort_order' => ($currentSort === 'siswa.nama_lengkap' && $currentOrder === 'asc') ? 'desc' : 'asc'])); ?>" 
                                           class="text-gray-400 hover:text-gray-600 transition-colors">
                                            <?php if($currentSort === 'siswa.nama_lengkap'): ?>
                                ...
                                <th scope="col" class="px-6 py-3 text-center text-xs font-medium text-gray-500 uppercase tracking-wider">
                                    Kelas
                                    <div class="inline-flex ml-2">
                                        <a href="<?php echo e(request()->fullUrlWithQuery(['sort_by' => 'kelas.nama_kelas', 'sort_order' => ($currentSort === 'kelas.nama_kelas' && $currentOrder === 'asc') ? 'desc' : 'asc'])); ?>" 
                                           class="text-gray-400 hover:text-gray-600 transition-colors">
                                            <?php if($currentSort === 'kelas.nama_kelas'): ?>
                                                <?php if($currentOrder === 'asc'): ?>
                                                    <i class="fas fa-sort-up text-blue-500"></i>
                                                <?php else: ?>
                                                    <i class="fas fa-sort-down text-blue-500"></i>
                        ...
                        </thead><tbody class="bg-white divide-y divide-gray-200">                            <?php $__empty_1 = true; $__currentLoopData = $siswa; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $index => $s): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                            <tr>
                                <td class="px-6 py-4 whitespace-nowrap text-center">
                                    <input type="checkbox" name="ids[]" value="<?php echo e($s->id); ?>" class="row-checkbox form-checkbox h-4 w-4 text-blue-600">
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-center">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-center">
                                    <div class="flex justify-center">
                                        <div class="h-12 w-12 rounded-full overflow-hidden bg-gray-100 flex-shrink-0">
                                            <?php if($s->foto && Storage::disk('public')->exists($s->foto)): ?>
                                                <img src="<?php echo e(asset('storage/' . $s->foto)); ?>" alt="Foto <?php echo e($s->nama_lengkap); ?>" class="h-full w-full object-cover">
                                            <?php else: ?>
                                                <img src="https://ui-avatars.com/api/?name=<?php echo e(urlencode($s->nama_lengkap)); ?>&background=3b82f6&color=ffffff" alt="Foto <?php echo e($s->nama_lengkap); ?>" class="h-full w-full object-cover">
                                            <?php endif; ?>
                                        </div>
                                    </div>
                                </td>
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-center">
                                    <div class="text-sm text-gray-900"><?php echo e($s->kelas->nama_kelas ?? 'Belum ada kelas'); ?></div>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-center">
                                    <div class="text-sm text-gray-900"><?php echo e($s->telepon ?? '-'); ?></div>
                                </td>
                                <td class="px-6 py-4 text-center">
                                    <div class="text-sm text-gray-900">
                                        <?php if($s->alamat): ?>
                                            <?php if(strlen($s->alamat) > 50): ?>
                                                <div><?php echo e(substr($s->alamat, 0, 50)); ?></div>
                                                <div class="text-gray-500"><?php echo e(substr($s->alamat, 50)); ?></div>
                                            <?php else: ?>
                                                <?php echo e($s->alamat); ?>
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-center">
                                    <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full <?php echo e($s->status == 'aktif' ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'); ?>">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-center">
                                    <div class="flex justify-center space-x-2">
                                        <a href="<?php echo e(route('admin.siswa.show', $s->id)); ?>" class="text-blue-600 hover:text-blue-900">
                                            <i class="fas fa-eye"></i>
                                        </a>
                                        <a href="<?php echo e(route('admin.siswa.edit', $s->id)); ?>" class="text-yellow-600 hover:text-yellow-900">
                                            <i class="fas fa-edit"></i>
                                        </a>
                                        <form action="<?php echo e(route('admin.siswa.destroy', $s->id)); ?>" method="POST" class="inline-block delete-form">
                                            <?php echo csrf_field(); ?>
                                            <?php echo method_field('DELETE'); ?>
                                            <button type="button" class="text-red-600 hover:text-red-900 delete-btn" data-id="<?php echo e($s->id); ?>" data-name="<?php echo e($s->nama_lengkap); ?>">
                            ...
                            <tr>
                                <td colspan="9" class="px-6 py-4 whitespace-nowrap text-center text-gray-500">
                                    <div class="flex flex-col items-center py-5">
                                        ...
                                        <div class="mt-4">
                                            <a href="<?php echo e(route('admin.siswa.create')); ?>" class="inline-flex items-center px-4 py-2 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-blue-600 hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500">
                    ...
                    <div class="px-6 py-4 border-t border-gray-200 flex items-center justify-between">
                        <div class="flex items-center space-x-4">
                            ...
                            <div class="text-sm text-gray-700">
                                Menampilkan semua data siswa (<?php echo e($siswa->count()); ?> data)
                            </div>
                        </div>
...
<div id="importUploadModal" class="hidden fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full z-50">
    <div class="relative top-20 mx-auto p-5 border w-11/12 md:w-2/3 lg:w-1/2 shadow-lg rounded-md bg-white">
        ...
        <div class="p-6">
            <!-- Upload Form -->
            <form id="modalImportForm" action="<?php echo e(route('admin.siswa.import')); ?>" method="POST" enctype="multipart/form-data">
                <?php echo csrf_field(); ?>
...
<div id="importInfoModal" class="hidden fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full z-50">
    <div class="relative top-20 mx-auto p-5 border w-11/12 md:w-4/5 lg:w-3/4 xl:w-2/3 shadow-lg rounded-md bg-white">
        ...
        <div class="flex items-center justify-between p-4 border-t border-gray-200 bg-gray-50">
            ...
            <div class="flex space-x-3">
                <a href="<?php echo e(route('admin.siswa.template', ['format' => 'excel'])); ?>" 
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function deleteStudent(studentId, tableRow) {
        ...
        fetch(`<?php echo e(url('/admin/siswa')); ?>/${studentId}`, {
            method: 'DELETE',
            headers: {
...
<?php echo $__env->make('layouts.admin', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\wamp64\www\website-smk3\resources\views/admin/siswa/index.blade.php ENDPATH**/ ?>