<?php $__env->startSection('title', 'Dashboard Guru - SMK PGRI CIKAMPEK'); ?>
...
<div class="p-3 sm:p-4 lg:p-6">
        ...
        <div class="relative p-4 sm:p-6 lg:p-8">
            ...
            <div class="relative z-10">
                <h1 class="text-xl sm:text-2xl lg:text-3xl font-bold text-white mb-2">
                    Selamat <?php echo e(now()->format('H') < 10 ? 'Pagi' : (now()->format('H') < 15 ? 'Siang' : (now()->format('H') < 18 ? 'Sore' : 'Malam'))); ?>, <?php echo e(auth()->guard('guru')->user()->nama_lengkap); ?>!
                </h1>
                <p class="text-blue-100 text-sm sm:text-base"><?php echo e(\Carbon\Carbon::now()->locale('id')->isoFormat('dddd, D MMMM Y')); ?></p>
                
                <?php if(isset($kelasWali) && $kelasWali->count() > 0): ?>
                <div class="mt-3 sm:mt-4 inline-flex items-center px-3 sm:px-4 py-2 bg-white bg-opacity-20 text-white rounded-lg text-sm">
                    ...
                    <span>Anda adalah wali kelas <?php echo e($kelasWali->pluck('nama_kelas')->join(', ')); ?></span>
                </div>
                <?php endif; ?>
            </div>
        </div>
    ...
    <div class="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4 lg:gap-6 mb-8">
        <!-- Total Jam Mengajar -->
        <div class="bg-white rounded-lg shadow p-2 sm:p-3 lg:p-4 dashboard-card border-l-4 border-blue-500 cursor-pointer" onclick="window.location.href='<?php echo e(route('guru.jadwal.index')); ?>'">
            <div class="flex flex-col lg:flex-row lg:justify-between">
                <div class="mb-1 lg:mb-0">
                    <h3 class="text-xs sm:text-sm lg:text-xs font-medium text-gray-500">Total Jam Mengajar</h3>
                    <p class="text-lg sm:text-xl lg:text-lg font-bold text-gray-800"><?php echo e(isset($jadwal) ? $jadwal->count() : 0); ?> <span class="hidden sm:inline">Jam</span></p>
                </div>
            ...
            <div class="mt-1 hidden sm:block">
                <a href="<?php echo e(route('guru.jadwal.index')); ?>" class="text-blue-600 hover:text-blue-800 text-xs sm:text-sm font-medium">
                    Lihat Detail →
                </a>
            </div>
        </div>
        ...
        <!-- Kelas Diampu -->
        <div class="bg-white rounded-lg shadow p-2 sm:p-3 lg:p-4 dashboard-card border-l-4 border-green-500 cursor-pointer" onclick="window.location.href='<?php echo e(route('guru.kelas.index')); ?>'">
            <div class="flex flex-col lg:flex-row lg:justify-between">
                <div class="mb-1 lg:mb-0">
                    <h3 class="text-xs sm:text-sm lg:text-xs font-medium text-gray-500">Kelas Diampu</h3>
                    <p class="text-lg sm:text-xl lg:text-lg font-bold text-gray-800"><?php echo e(isset($kelasDiajar) ? $kelasDiajar->count() : 0); ?> <span class="hidden sm:inline">Kelas</span></p>
                </div>
            ...
            <div class="mt-1 hidden sm:block">
                <a href="<?php echo e(route('guru.kelas.index')); ?>" class="text-green-600 hover:text-green-800 text-xs sm:text-sm font-medium">
                    Lihat Detail →
                </a>
            </div>
        </div>
        ...
        <!-- Absensi -->
        <div class="bg-white rounded-lg shadow p-2 sm:p-3 lg:p-4 dashboard-card border-l-4 border-purple-500 cursor-pointer" onclick="window.location.href='<?php echo e(route('guru.absensi.index')); ?>'">
            <div class="flex flex-col lg:flex-row lg:justify-between">
                <div class="mb-1 lg:mb-0">
            ...
            <div class="mt-2 hidden sm:block">
                <a href="<?php echo e(route('guru.absensi.index')); ?>" class="text-purple-600 hover:text-purple-800 text-sm sm:text-base font-medium">
                    Kelola Absensi →
                </a>
            </div>
        </div>
        ...
        <?php if(isset($kelasWali) && $kelasWali->count() > 0): ?>
        <div class="bg-white rounded-lg shadow p-2 sm:p-3 lg:p-4 dashboard-card border-l-4 border-amber-500 cursor-pointer" onclick="window.location.href='<?php echo e(route('guru.wali-kelas.dashboard')); ?>'">
            <div class="flex flex-col lg:flex-row lg:justify-between">
                <div class="mb-1 lg:mb-0">
                    <h3 class="text-xs sm:text-sm lg:text-xs font-medium text-gray-500">Wali Kelas</h3>
                    <p class="text-lg sm:text-xl lg:text-lg font-bold text-gray-800"><?php echo e($kelasWali->first()->nama_kelas); ?></p>
                </div>
            ...
            <div class="mt-1 hidden sm:block">
                <a href="<?php echo e(route('guru.wali-kelas.dashboard')); ?>" class="text-amber-600 hover:text-amber-800 text-xs sm:text-sm font-medium">
                    Dashboard Wali Kelas →
                </a>
            </div>
        </div>
        <?php else: ?>
        <div class="bg-white rounded-lg shadow p-2 sm:p-3 lg:p-4 dashboard-card border-l-4 border-amber-500 cursor-pointer" onclick="window.location.href='<?php echo e(route('guru.kelas.index')); ?>'">
            <div class="flex flex-col lg:flex-row lg:justify-between">
                <div class="mb-1 lg:mb-0">
            ...
            <div class="mt-1 hidden sm:block">
                <a href="<?php echo e(route('guru.kelas.index')); ?>" class="text-amber-600 hover:text-amber-800 text-xs sm:text-sm font-medium">
                    Daftar Kelas →
                </a>
            </div>
        </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-4 sm:gap-6 mt-6 sm:mt-8">
        ...
        <div class="lg:col-span-2">
            <div class="bg-white rounded-lg shadow-md">
                <div class="p-3 sm:p-5 border-b border-gray-100">
                    <div class="flex flex-col sm:flex-row sm:justify-between sm:items-center">
                        ...
                        <div class="px-3 py-1 text-xs bg-blue-100 text-blue-800 rounded-lg self-start">
                            <?php echo e(\Carbon\Carbon::now()->locale('id')->isoFormat('dddd')); ?>
                ...
                <div class="p-3 sm:p-5">
                    ...
                    <?php if($groupedJadwal->count() > 0): ?>
                        <div class="space-y-3">
                            <?php $__currentLoopData = $groupedJadwal; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $group): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                            <?php
                            ...
                            <div class="bg-blue-50 rounded-lg p-3 hover:bg-blue-100 transition-colors overflow-hidden">
                                <div class="flex items-start space-x-3">
                                    ...
                                    <div class="flex-1 min-w-0 overflow-hidden">
                                        <h4 class="text-sm font-medium text-gray-900 line-clamp-2 leading-tight"><?php echo e($group['mapel'] ? $group['mapel']->nama : 'Mata Pelajaran tidak ditemukan'); ?></h4>
                                        <p class="text-xs text-gray-600 mt-1">
                                            <?php echo e(substr($group['jam_mulai'], 0, 5)); ?> - <?php echo e(substr($group['jam_selesai'], 0, 5)); ?>

                                            <?php if(count($group['items']) > 1): ?>
                                            <span class="text-blue-600 font-medium">(<?php echo e(count($group['items'])); ?> JP)</span>
                                            <?php endif; ?>
                                        </p>
                                        <div class="flex items-center mt-1 overflow-hidden">
                                            ...
                                            <p class="text-xs text-gray-700 truncate"><?php echo e($group['kelas'] ? $group['kelas']->nama_kelas : 'Kelas tidak ditemukan'); ?></p>
                                        </div>
                                    </div>
                                    <div class="flex flex-col items-end space-y-2 flex-shrink-0">
                                        ...
                                        <?php endif; ?>
                                        <a href="<?php echo e(route('guru.absensi.create', ['jadwal_id' => $group['items'][0]->id, 'kelas_id' => $group['kelas']->id, 'mapel_id' => $group['mapel']->id])); ?>"
            ...
            <div class="bg-white rounded-lg shadow-md mt-4 sm:mt-6" id="daftar-kelas">
                ...
                <div class="p-3 sm:p-5">
                    <?php if(isset($kelasDiajar) && $kelasDiajar->count() > 0): ?>
                    <div class="grid grid-cols-2 gap-3 sm:gap-4">
                        ...
                        <div class="bg-gray-50 rounded-lg p-2 sm:p-3 hover:bg-blue-50 transition-all">
                            <div class="flex items-center mb-2">
                                ...
                                <div>
                                    <h4 class="font-medium text-gray-900 text-xs sm:text-sm"><?php echo e($kelas->nama_kelas); ?></h4>
                                    <p class="text-xs text-gray-600"><?php echo e($kelas->jurusan->nama_jurusan ?? 'Jurusan tidak tersedia'); ?></p>
                                </div>
                            </div>
        ...
        <div class="lg:col-span-1">
            ...
            <div class="bg-white rounded-lg shadow-md">
                ...
                <div class="p-3 sm:p-5">
                    <?php if(isset($pengumuman) && $pengumuman->count() > 0): ?>
                    <div class="space-y-3 sm:space-y-4">
                        ...
                        <div class="border-l-4 border-blue-500 pl-3 py-1">
                            <h4 class="font-medium text-gray-900 text-sm sm:text-base"><?php echo e($p->judul); ?></h4>
                            <p class="text-xs sm:text-sm text-gray-600"><?php echo e(\Illuminate\Support\Str::limit(strip_tags($p->isi), 70)); ?></p>
                            <div class="mt-2 flex items-center justify-between">
                                <span class="text-xs text-gray-500"><?php echo e($p->created_at->diffForHumans()); ?></span>
                                <a href="<?php echo e(route('guru.pengumuman.show', $p->id)); ?>" class="text-blue-600 hover:text-blue-800 text-xs sm:text-sm">Baca</a>
                            </div>
                        </div>
                        <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                    </div>
                    ...
                    <div class="mt-3 sm:mt-4 text-center">
                        <a href="<?php echo e(route('guru.pengumuman.index')); ?>" class="text-blue-600 hover:text-blue-800 text-xs sm:text-sm font-medium">
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        const calendarEvents = <?php echo json_encode($calendarEvents ?? [], 15, 512) ?>;
        ...
        async function fetchCalendarEvents(year, month) {
            try {
                const response = await fetch(`<?php echo e(route('guru.calendar.events')); ?>?year=${year}&month=${month}`);
                if (response.ok) {
...
<?php $__env->stopPush(); ?>
<?php echo $__env->make('layouts.guru', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\wamp64\www\website-smk3\resources\views/guru/dashboard.blade.php ENDPATH**/ ?>